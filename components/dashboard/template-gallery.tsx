  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-muted">{TEMPLATES.length} modèles disponibles</span>
      </div>
        ...
        {TEMPLATES.map((template) => {
          ...
          return (
            <button
              ...
              disabled={isLocked}
              className={`group relative rounded-xl overflow-hidden border-2 transition-all duration-200 text-left cursor-pointer ${
                isSelected
                  ? 'border-accent shadow-lg shadow-accent/15 scale-[1.02]'
                  : isLocked
            ...
            >
              <div className="relative aspect-[3/4] bg-background overflow-hidden">
                ...
                {isSelected && (
                  <div className="absolute inset-0 bg-accent/10 flex items-center justify-center">
                    <div className="bg-accent rounded-full p-1.5 shadow-lg shadow-accent/30">
                ...
                {isLocked && (
                  <div className="absolute inset-0 bg-background/60 backdrop-blur-[1px] flex flex-col items-center justify-center gap-1.5">
                ...
                {hoveredTemplate === template.id && !isLocked && !isSelected && (
                  <div className="absolute inset-0 bg-background/40 flex items-center justify-center transition-opacity">
              ...
              <div className="p-2.5 bg-surface">
                <div className="flex items-center justify-between">
                  <span className={`text-xs font-bold ${isSelected ? 'text-accent-soft' : 'text-foreground'}`}>
                    {template.name}
                  </span>
                  {template.premium && (
                ...
                <div className="flex gap-1 mt-1.5">
                  {template.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-[8px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded bg-surface-alt text-muted"
                    >
                      {tag}
                    </span>
                  ))}
